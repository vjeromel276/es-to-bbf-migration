...
with field matching and picklist value mapping
"""
...
def create_picklist_mapping(es_picklists, bbf_picklists, field_mapping_df):
    """Create picklist mapping sheet data"""
    picklist_data = []
    ...
    for bbf_field in bbf_picklist_fields:
        ...
        if field_match.empty or not field_match.iloc[0]['ES_Field_API_Name']:
            continue

        es_field = field_match.iloc[0]['ES_Field_API_Name']

        # Check if ES field exists in ES picklists
        if es_field not in es_picklists['Field API Name'].values:
            continue
...
def main():
    ...
    picklist_mapping_df = create_picklist_mapping(es_picklists, bbf_picklists, field_mapping_df)

    if not picklist_mapping_df.empty:
        print(f"Total picklist values mapped: {len(picklist_mapping_df)}")
        ...
        print("No picklist mappings found")
