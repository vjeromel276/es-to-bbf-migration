...
with field matching and picklist value mapping (Version 2 - Enhanced Picklist Mapping)
"""
...
def create_picklist_mapping(es_picklists, bbf_picklists, field_mapping_df, es_fields):
    """Create picklist mapping sheet data - Enhanced to handle semantic field matches"""
    picklist_data = []
    ...
    for bbf_field in bbf_picklist_fields:
        ...
        if not field_match.empty:
            ...
            print(f"  Found ES match from field mapping: {es_field}")

            if not es_field or es_field not in es_picklists['Field API Name'].values:
                print(f"  ES field '{es_field}' not found in ES picklists or is empty")
                # If no direct match but BBF field is picklist, show BBF field alone
                if bbf_field in bbf_picklists['Field API Name'].values:
            ...
            print(f"  ES values: {len(es_values)}, BBF values: {len(bbf_values)}")
            ...
            print(f"  No field mapping found for {bbf_field}")
            # If BBF picklist field has no match, show all BBF values for manual mapping
            if bbf_field in bbf_picklists['Field API Name'].values:
                bbf_values = bbf_picklists[bbf_picklists['Field API Name'] == bbf_field]['Picklist Value'].tolist()
                picklist_data.append({
                    'ES_Field': '',
...
def main():
    ...
    print(f"BBF fields after filtering: {len(field_mapping_df)}")
    ...
    if not picklist_mapping_df.empty:
        print(f"\nTotal picklist values mapped: {len(picklist_mapping_df)}")
